<?php
class Categorias extends CategoriasModelo
{
    private $banner;
    ...
    public function __construct()
    {
        ...
        $this->ruta_amigable_paginacion = null;
        $this->total_articulos = null;
    ...
    {
        $this->categoria = explode(",", $parametros)[0];
        $this->pagina_actual = (isset(explode(",", $parametros)[1])) ? explode(",", $parametros)[1] : 1;
        $this->parametros = $this->categoria . "," . $this->pagina_actual;
        $this->get_datos($this->categoria);
    }
    public function get_datos($categoria)
    {
        if ($this->get_datos_categoria($categoria)) {
            if ($this->get_palabras_claves($categoria)) {
                if ($this->get_anuncios($categoria)) {
                    if ($this->get_banner()) {
                        if ($this->get_articulos($this->parametros)) {
                            if ($this->get_articulos_destacados_recientes($categoria)) {
                                return true;
                            } else {
                                return false;
                            }
    ...
    public function get_palabras_claves($categoria)
    {
        if ($this->get_palabras_claves_modelo($categoria)) {
    ...
    public function get_articulos($parametros)
    {
        $this->set_atributo('parametros', explode(",", $parametros));
        $this->set_atributo('categoria', $this->parametros[0]);
        $this->set_atributo('pagina', $this->parametros[1]);
        $this->set_atributo('ruta_amigable_paginacion', "Categorias/inicio/" . $this->categoria . "/");
        $this->get_total_articulos($this->categoria);
        $this->set_atributo('pagina_actual', $this->pagina);
        $this->set_atributo('total_paginas', ceil($this->total_articulos / $this->cantidad));
        $this->set_atributo('desde', ($this->pagina - 1) * $this->cantidad);
        if ($this->pagina_actual > 5 && $this->pagina_actual <= $this->total_paginas) {
            ...
            if ($this->total_paginas < 5) {
                ...
                $this->set_atributo('ultima_pagina', $this->total_paginas);
            } else {
                $this->set_atributo('primera_pagina', 0);
